...
namespace Mag{
    ...
    public class Population{
        ...
        public void Evolve(){
            ...
            File.AppendAllText("Results/curriculum.csv", $"\nFirst:{population[0].WriteToString()}", Encoding.UTF8);
        ...
        private void PrintEpochResults(int epoch){
            ...
            Console.WriteLine($"    indiv fitness={population[0].fitness}");
            csv.AppendLine();
        ...
        private void WriteResultsToFile(){
                ...
                for(int pair=0; pair<schedule[groupId].GetLength(1); pair++){
                    for(int weekDay=0; weekDay<schedule[groupId].GetLength(0); weekDay++){
                        if(schedule[groupId][weekDay, pair] != null){
                            ...
                            scheduleData.Append($"{d.discipline}, {DBLink.Teachers[d.teacherId].teacherName}, {DBLink.Audiences[d.auditoriumId].audienceNumber}; ");
                        }
                        else{
                            scheduleData.Append($" - ;");
                        }
            ...
            Console.Write(scheduleData.ToString());
            File.AppendAllText("Results/schedule.csv", scheduleData.ToString(), Encoding.UTF8);
            File.AppendAllText("Results/curriculum.csv", $"\nLast:{population[0].WriteToString()}", Encoding.UTF8);
            File.AppendAllText("Results/output.csv", csv.ToString(), Encoding.UTF8);
        }
